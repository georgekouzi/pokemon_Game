...
public class GUI extends JFrame{
	 ...
	 */
	public GUI(game_service game,int scenaio)
	{
		try {
	...
	private void drawScore(Graphics g){
		...
		g.drawString("Total Score: "+String.valueOf(score), this.getWidth()-280, 70);
		}
		}
	...
	private  void drawPokemons(Graphics g) {
		...
		if(fs!=null) {
			...
			while(itr.hasNext()) {
				...
				if(fp!=null) {
					if (f.getType()==-1) {
						g.drawImage(charmanderimg,(int)fp.x()-20, (int)fp.y()-121,120, 120, null);
					}
					else if (f.getType()==1) {
						g.drawImage(squiltelimg,(int)fp.x()-20, (int)fp.y()-121,120, 120, null);
					}
					else {
						g.drawImage(balbazurimg,(int)fp.x()-20, (int)fp.y()-121,120, 120, null);
					}
				}
			}
		}
	  ...
	  private void drawscenario(Graphics g) {
	        ...
	        g.drawString("Level: "+scenaio,this.getWidth()/2 - 40, 20);
	    }
	/**
	 ...
	 *
	 */
	   private void agantscoreupdate(Graphics g){
	        List<CL_Agent> agents = _ar.getAgents();
	        for(CL_Agent agent : agents)
	        {
	            g.setFont(new Font("Arial",Font.BOLD,20));
	            g.drawString("agent "+agent.getID()+": "+String.valueOf(agent.getValue()), this.getWidth()-180, 90+20*agent.getID());
	        }
	       
	    }
	   
	private void drawEdge(edge_data e, Graphics g,List<Double> wieght) {
        ...
        g2.draw(new Line2D.Float((int)s0.x()-10, (int)s0.y()-100, (int)d0.x()-10, (int)d0.y()-100));
...
//		g.drawLine((int)s0.x()-12, (int)s0.y()-102, (int)d0.x()-12, (int)d0.y()-102);
		...
		Double wi=gg.getEdge(e.getSrc(), e.getDest()).getWeight();
		wi=Double.parseDouble(new DecimalFormat("##.##").format(wi));
		int vegX = ((int)(s0.x())+(int)(d0.x()-62))/2 ;
		int vegY = ((int)(s0.y()-102)+(int)(d0.y()-102))/2;
		if (!wieght.contains(wi)) {
			g2.drawString(""+wi,vegX-10,vegY+30);
			wieght.add(wi);
		}
	}
	...
	private void drawGraph(Graphics g) {
		List<Double> wieght = new ArrayList<>();
		directed_weighted_graph gg = _ar.getGraph();
